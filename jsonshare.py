...
class JsonShareDB(collections_abc.MutableMapping):
    """ The main 'connection' object for JsonShare database """
    ...
    def __getitem__(self, key):
        ...
        try:
            ...
            with fil.open("rb") as f:
                # TODO: what's the relative merit of this vs json.load(f)?
    ...
    def getlink(self, folder):
        ...
        return JsonShareLink(self, folder)

    def __repr__(self):
        return "JsonShareDB('%s')" % self.root


class JsonShareLink:
    """ A shorthand for accessing nested JsonShare data conveniently.

    Created through JsonShareDB.getlink(), example::
...
def main():
    ...
    usage = textwrap.dedent(
        """\
    jsonshare - manage JsonShare databases

    Usage:

        jsonshare dump /path/to/db > dump.txt
        jsonshare load /path/to/db < dump.txt
        jsonshare test /path/to/db
    """
    )
    DB = JsonShareDB
    import sys
